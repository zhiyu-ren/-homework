...
def init_db():
    ...
    db.execute("""CREATE TABLE IF NOT EXISTS users(
        ...
        avatar TEXT DEFAULT NULL,created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
def upload():
    ...
    if request.method=="POST":
        ...
        if ext not in ALLOWED_EXTENSIONS:
            logger.warning("上传被拒:扩展名%s(用户:%s)",ext,username)
            return render_template("upload.html",error=f"不允许上传{ext}文件，仅允许JPG/PNG/GIF")
        if file.content_type not in ALLOWED_MIMES:
            logger.warning("上传被拒:MIME%s(用户:%s)",file.content_type,username)
        ...
        if real_ext is None:
            os.remove(tmp_path)
            logger.warning("上传被拒:幻数校验失败(用户:%s)",username)
            return render_template("upload.html",error="文件内容不合法，请上传真实图片")
        ...
        logger.info("头像上传成功:%s(用户:%s)",final_name,username)
        return render_template("upload.html",success=True,file_url=file_url,filename=final_name)
    return render_template("upload.html")

@app.route("/health")
def health():
    return jsonify({"status":"ok","version":"4.0.0-secure"}),200
...
if __name__=="__main__":
    logger.info("安全版(含上传修复)启动:0.0.0.0:5000")
