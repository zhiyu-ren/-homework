...
| 🟢 **Day 10** | [`Class10/`](./Class10) | 实习报告（10天学习总结） | Word / 安全审计 |
...
|:----|:------|
| 一、实习目的、要求 | 实训目标与基本要求 |
| 二、实习主要内容 | Day01-Day09 逐天学习内容总结 |
| 三、漏洞技术总结 | 8大类漏洞汇总表 + 密码/认证/输入验证安全 |
| 四、实习总结 | 个人收获与心得体会 |
...
|:----|:----:|:---------|
| SQL注入 | 🔴 严重 | 参数化查询（?占位符） |
| 文件上传/一句话木马 | 🔴 严重 | 扩展名白名单+幻数校验 |
| 水平越权(IDOR) | 🔴 严重 | Session获取用户身份 |
| 负值交易 | 🟠 高危 | amount > 0校验 |
| 路径遍历 | 🔴 严重 | 白名单机制 |
| CSRF | 🟡 中危 | CSRF Token + HMAC验证 |
| SSTI模板注入 | 🔴 严重 | 模板变量传递 |
| 命令注入（反弹Shell） | 🔴 严重 | shlex.quote()+shell=False |

[📄 完整实习报告 →](./Class10/四川大学本科实习报告_任治宇.docx)
